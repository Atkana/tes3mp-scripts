...
config.defaultPrice = 100000 --The price a house defaults to when it's created
...
local function removeHouseOwner(houseName, refund, furnReturn)
	...
	if kanaFurniture ~= nil then
		for cellName, v in pairs(hdata.cells) do
			...
			-- Lear edit start	-	Attempt to remove marketPlace for players upon removal.
			jcMarketplace.marketDelete(cellName)
			-- Lear edit end.
...
local function getHouseInfoLong(houseName, toggleMeanings) --Used in GUI labels
	...
	if getHouseOwnerName(hdata.name) then
		...
		-- added the below.
		text = text .. color.MsgBox .. "The house is worth " .. hdata.price .. " gold, but is currently " .. color.Red .. "owned " .. color.MsgBox .."by " .. owner .. ".\n"
	else
		text = text .. color.MsgBox .. "The house is worth " .. hdata.price .. " gold, and is " .. color.Green .. "available " .. color.MsgBox .. "for purchase.\n"
		-- added the above.
	end
...
local function onDirtyThief(pid)
	...
	elseif Players[pid].data.settings.staffRank < 2 then
		...
		--local karmaMessage = color.MsgBox .. logicHandler.GetChatName(pid) .. " has received " .. 
		--			color.Red .. " very bad " .. color.MsgBox .. "karma for stealing from a player house.\n"
			...
			if placed then
				for refIndex, v2 in pairs(placed) do
					if v2.owner == getName(pid) then
						placedNum = placedNum + 1
						--placedSellback = placedSellback + kanaFurniture.GetSellBackPrice(kanaFurniture.GetFurnitureDataByRefId(v2.refId).price)
						placedSellback = placedSellback + kanaFurniture.GetSellBackPrice(kanaFurniture.GetFurnitureDataByRefId(v2.refId:lower()).price)
						--string.lower(oname)
					end
				end
			end
...
showPlayerSettingsMain = function(pid)
	...
	if playerSelectedHouse[getName(pid)] then
		...
		message = message .. color.MsgBox .. "The house is currently "
		if isLocked(hdata.name) then
			message = message .. color.Red .. "locked" .. color.MsgBox .. ".\n"
		else
			message = message .. color.Green .. "unlocked" .. color.MsgBox .. ".\n"
		end
		--TODO: More?
	end
...
local function onPlayerSelectWarp(pid)
	...
	-- Lear edit start - Disable house warping when in prison.
	if tes3mp.GetCell(pid) == "Character Stuff Wonderland" then
		tes3mp.SendMessage(pid, color.Red .. "You cannot warp from this location.\n", false)
		return false
	end
	
	-- Disable warp if pvp is enabled:
	if Players[pid].data.customVariables.npCustom.pvp.activePVP ~= nil and Players[pid].data.customVariables.npCustom.pvp.activePVP == 1 then
		tes3mp.SendMessage(pid, color.Red .. "You cannot warp while you are PVP flagged.\n", false)
		return false
	end
...
showAllHousesList = function(pid)
	...
	for houseName, v in pairs(housingData.houses) do
		...
		local owner = color.Green .. " -^- Available"
		
		if getHouseOwnerName(houseName) then
			--owner = color.Red .. " -^- Sold"
			owner = color.Red .. " -^- Owned by " .. getHouseOwnerName(hdata.name) 
		end	

			table.insert(options, houseName)
			table.insert(options, owner)
...
local function onHouseEditOwnerPrompt(pid, data)
	...
	if data == nil or data == "" or string.lower(data) == "none" then
		...
		-- Lear edit start	-	Begin edit for marketplace removal.
		local cellName = adminSelectedHouse[getName(pid)]
		jcMarketplace.marketDelete(cellName)
		-- Lear edit end.
	else
		...
		if housingData.cells[currentCell] and housingData.cells[currentCell].house ~= nil then
			...
			if enterReason == "unowned" then --A player has entered a house without an owner
				if lastEnteredHouse[getName(pid)] ~= hdata.name then --They've just entered the house
					msg(pid, color.MsgBox .. "You've entered " .. hdata.name .. ", which is a house " .. color.Green .."available " .. color.MsgBox .. "for purchase for " .. hdata.price .. " gold. Type " .. color.Yellow .."/houseinfo " .. color.MsgBox .. "for more info.")
				end
			...
			elseif enterReason == "owner" or enterReason == "coowner" then --An owner/coowner has entered
				if lastEnteredHouse[getName(pid)] ~= hdata.name then --They've just entered the house
					local message = color.MsgBox .. "Welcome home, " .. Players[pid].accountName .. ". "
					if isLocked(hdata.name) then
						message = message .. "The house is still " .. color.Red .. "locked " .. color.MsgBox .. "to outsiders."
					end
					msg(pid, message)
				end
			...
			elseif enterReason == "access" then
				local message = color.MsgBox .. "You've just entered part of " .. getHouseOwnerName(hdata.name) .. "'s house. The house is currently " .. color.Red .. "locked" .. color.MsgBox .. ", but you're allowed in here because it's marked as important for a quest. Please respect " .. getHouseOwnerName(hdata.name) .. "'s property, and only do what you must for the quest."
				msg(pid, message)
			...
			elseif canEnter == false then
				msg(pid, color.MsgBox .. "The owner has " .. color.Red .. "locked " .. color.MsgBox .. "the house.")
				local destinationCell, destinationPos
				if hdata.outside.cell then --There's exit data saved for this house
					destinationCell = hdata.outside.cell
					destinationPos = hdata.outside.pos
